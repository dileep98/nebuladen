    ...
    (async () => {
      ...
      try {
        ...
        const stats = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/agent/status`,
          {
    ...
    <main className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        ...
        <p className="text-white/50 mb-10">
          Your personal Nebula is ready and waiting for instructions.
        </p>

        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 flex items-center justify-between">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-2xl bg-violet-500/20 border border-violet-500/30 flex items-center justify-center text-3xl">
              🌌
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <h2 className="text-xl font-semibold">
                  Nebula-{user.id?.slice(0, 6) || "001"}
                </h2>
              ...
              <p className="text-white/40 text-sm">
                Personal agent · Cloud compute ready
              </p>
            </div>
          </div>
          <button
            onClick={() => router.push("/chat")}
            className="px-6 py-3 bg-violet-600 hover:bg-violet-500 rounded-xl font-medium transition text-sm"
          >
            Open Chat →
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          {[
            ...
            { label: "Region", value: agentStats.region ?? "us-east-1" },
          ].map((stat) => (
            <div
              key={stat.label}
